...
enum class ClickType(val mode: Mode, val button: Int, val slot: Int = -1, val bukkitId: String = "UNKNOWN") {
    ...
    companion object {
        ...
        fun findBukkit(bukkitId: String, hotbar: Int): ClickType {
            return entries
                .find { it.bukkitId == bukkitId && if (bukkitId == "NUMBER_KEY") hotbar == it.button else true }
                ?: error("Unknown click type from bukkit: $bukkitId ($hotbar)")
        }

        fun find(mode: Mode, button: Int, slot: Int = -1): ClickType? {
            return entries
                .find { it.mode == mode && it.button == button && (it.slot == -1 || it.slot == slot) }
                .also {
                    if (it == null) {
                        info("Not found interact type. Mode: $mode, Button: $button, Slot: $slot")
                    }
            }
        }
